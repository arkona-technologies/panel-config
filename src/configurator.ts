...
import { Configuration } from "./utils/types.js";

const path = process.argv[2];
const configpath = path ? path : `${__dirname.replace(/\/$/, "")}/../configurations/config.json`;
const data = JSON.parse(fs.readFileSync(configpath, "utf-8")) as Configuration;
const schema = JSON.parse(
  fs.readFileSync(`${__dirname.replace(/\/$/, "")}/schema.json`, "utf-8")
);
const ajv = new Ajv();
...
export const CONFIGURATION = getConfig() as Configuration;
