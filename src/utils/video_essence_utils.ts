...
export function getVideoEssence(
  vm: VM.Any,
  ...
  if (vm instanceof AT1101.Root) {
    switch (essenceType) {
      case "r_t_p_receiver":
        v_src = enforce_nonnull(vm.r_t_p_receiver).video_receivers.row(
          essenceIndex
        ).media_specific.output.video;
        break;
      case "delay":
        v_src = enforce_nonnull(vm.re_play)
          .video.delays.row(essenceIndex)
          .outputs.row(0).video;
        break;
      case "player":
        v_src = enforce_nonnull(vm.re_play).video.players.row(essenceIndex)
          .output.video;
        break;
      case "video_signal_generator":
        v_src = enforce_nonnull(vm.video_signal_generator).instances.row(
          essenceIndex
        ).output;
        break;
      case "sdi":
        v_src = enforce_nonnull(vm.i_o_module).input.row(essenceIndex).sdi
          .output.video;
        break;
      case "cc1d":
        v_src = enforce_nonnull(vm.color_correction).cc1d.row(
          essenceIndex
        ).output;
        break;
      case "cc3d":
        v_src = enforce_nonnull(vm.color_correction).cc3d.row(
          essenceIndex
        ).output;
        break;
      case "video_mixer":
        v_src = enforce_nonnull(vm.video_mixer).instances.row(
          essenceIndex
        ).output;
        break;
      default:
...
function r_t_p_receiver_parser(suffix: string): ESSENCE_ENTRY {
  ...
  const match = suffix.match(regex);
  if (match && match[1])
    return { essenceIndex: parseInt(match[1]), essenceType: "r_t_p_receiver" };
  return { essenceIndex: -1, essenceType: "N/A" };
}
function i_o_module_parser(suffix: string): ESSENCE_ENTRY {
  ...
  const match = suffix.match(regex);
  if (match && match[1])
    return { essenceIndex: parseInt(match[1]), essenceType: "sdi" };
  return { essenceIndex: -1, essenceType: "N/A" };
}
function video_mixer_parser(suffix: string): ESSENCE_ENTRY {
  ...
  const match = suffix.match(regex);
  if (match && match[1])
    return { essenceIndex: parseInt(match[1]), essenceType: "video_mixer" };
  return { essenceIndex: -1, essenceType: "N/A" };
}
