...
import { ESSENCE_ENTRY, MIXER_CONFIG_ENTRY, PANEL_CONFIG, PROGRAM_PREVIEW_KEYER_MIXER_SET, VIDEO_MIXER_ENTRY } from "./utils/types.js";


export async function connectMachine(ip:string){
  try{
    //TODO login credentials
    const vmatrix = await VAPI.VM.open({
      ip:ip,
      towel:"videoMixerFunctions API (ง •̀_•́)ง",
      // login:{
      //   user:"",
      //   password:""
      // }
...
export async function gather_video_essences(vm:VAPI.VM.Any){
  let VideoEssenceCollectionT : ESSENCE_ENTRY[] = [];
  ...
  if(vm.r_t_p_receiver?.video_receivers != null){
    for(let e of await vm.r_t_p_receiver.video_receivers.rows()){
      VideoEssenceCollectionT.push({type:"rtp_receiver",index:e.index,essence:e.media_specific.output.video})
    }
  }
  ...
  if(vm.re_play?.video.players != null){
    for (let e of await vm.re_play?.video.players.rows()){
      VideoEssenceCollectionT.push({type:"delay_player",index:e.index,essence:e.output.video})
    }
  }

  //video signal generator
  if(vm.video_signal_generator !=null){
    VideoEssenceCollectionT.push({type:"video_signal_generator",index:0,essence:vm.video_signal_generator.instances.row(0).output})
  }
...
function getMixerModeShortcut(mode:VideoMixer.BSLKMode){
  switch(mode){
    case "LUMA_KEYER":return "Luma";
    case "LUMA_KEYER_ADDITIVE": return "Luma+";
    case "MIXER":
    case "MIXER_INDEPENDENT": return "Mixer";
    default:
      console.log(`Error: ${mode} is unknwon! (getMixerModeShortcut)`)
      return "UNKNWN";
  }
}
export async function gather_video_mixer_collection(vm:VAPI.VM.Any,mixerConfigCollection: MIXER_CONFIG_ENTRY[]){  
  ...
  for(const MIXER of await vm.video_mixer.instances.rows()){
    ...
    const LUMA_KEYER_BELONGINGS = mixerConfigCollection.filter(el=>el.belongingIndex === MIXER.index).map(el=>el.index);
    ...
    VideoMixerCollection.push({
      name:(MIXER_CONFIG.name==="")?`${getMixerModeShortcut(MIXER_CONFIG.mode)} ${MIXER.index}`:MIXER_CONFIG.name,
...
export function parseVideoEssence(essence:AT1130.Video.Essence | AT1101.Video.Essence | undefined | null):ESSENCE_ENTRY|null{
  ...
  switch(type){
    case "r_t_p_receiver":
      ...
      return {type:"rtp_receiver",index:parseInt(((rtpIndexString[0]==null)?"-1":rtpIndexString[0])),essence};
    case "color_correction":
      let colorIndexString = suffix.match(/(\d+)/);
      if(colorIndexString == null) return null;
      return {type:"color_correction",index:parseInt(((colorIndexString[0]==null)?"-1":colorIndexString[0])),essence};
    case "video_mixer":
      let videoMixerIndexString = suffix.match(/(\d+)/);
      if(videoMixerIndexString == null) return null;
      return {type:"video_mixer",index:parseInt(((videoMixerIndexString[0]==null)?"-1":videoMixerIndexString[0])),essence};
    case "video_signal_generator":
      let signalGeneratorIndexString = suffix.match(/(\d+)/);
      if(signalGeneratorIndexString == null) return null;
      return {type:"video_signal_generator",index:parseInt(((signalGeneratorIndexString[0]==null)?"-1":signalGeneratorIndexString[0])),essence};
    case "re_play":
      let videoDelayIndexString = suffix.match(/(\d+)/);
      if(videoDelayIndexString == null) return null;
      return {type:"delay_player",index:parseInt((videoDelayIndexString[0] == null)?"-1":videoDelayIndexString[0]),essence};
    default:
