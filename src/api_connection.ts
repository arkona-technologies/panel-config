...
import { AT1101, AT1130, type VideoMixer } from "vapi";
import type {
	OLD_ESSENCE_ENTRY,
	MIXER_CONFIG_ENTRY,
	PANEL_CONFIG,
...
export function getVsrc(
	...
	const timedSource: AT1130.Video.TimedSource | AT1101.Video.TimedSource = {
		source: null,
		switch_time: null,
	};
...
export async function gather_video_essences(vm: VAPI.VM.Any) {
	const VideoEssenceCollectionT: OLD_ESSENCE_ENTRY[] = [];
	...
	if (vm.video_mixer?.instances != null) {
		...
		for (const e of await vm.video_mixer.instances.rows()) {
	...
	if (vm.r_t_p_receiver?.video_receivers != null) {
		for (const e of await vm.r_t_p_receiver.video_receivers.rows()) {
			VideoEssenceCollectionT.push({
				type: "r_t_p_receiver",
				index: e.index,
	...
	if (vm.re_play?.video.players != null) {
		for (const e of await vm.re_play?.video.players.rows()) {
			VideoEssenceCollectionT.push({
				type: "re_play",
				index: e.index,
...
export async function gather_video_mixer_collection(
	...
	const VideoMixerCollection: VIDEO_MIXER_ENTRY[] = [];
	...
	for (const MIXER of await vm.video_mixer.instances.rows()) {
		...
		if (MIXER_CONFIG.mode === "LUMA_KEYER") {
			...
			if (MIXER_CONFIG.cascadingIndex != null) {
				const mixerSrc = vm.video_mixer.instances.row(
...
export function getSelectedMixer(panelConfig: PANEL_CONFIG) {
	...
	const mixer = mixerCollection.find((el) => el.index === selectedMixer);
	return mixer;
}
...
export function parseVideoEssence(
	...
	switch (type) {
		case "r_t_p_receiver":
			const rtpIndexString = suffix.match(/(\d+)/);
			if (rtpIndexString == null) return null;
			return {
				type: "r_t_p_receiver",
				index: Number.parseInt(
					rtpIndexString[0] == null ? "-1" : rtpIndexString[0],
				),
				essence,
			};
		case "color_correction":
			const colorIndexString = suffix.match(/(\d+)/);
		...
		case "video_mixer":
			const videoMixerIndexString = suffix.match(/(\d+)/);
		...
		case "video_signal_generator":
			const signalGeneratorIndexString = suffix.match(/(\d+)/);
		...
		case "re_play":
			const videoDelayIndexString = suffix.match(/(\d+)/);
