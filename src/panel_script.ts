...
import {
	PANEL_CHECK_ALIVE_INTERVAL,
	PANEL_CHECK_REVIVE_INTERVAL,
} from "./utils/global.js";
// import { Duration, pause } from 'vscript';

const main = async () => {
	//SECTION NEW
	const { machine_ip, panel_ip, panel_layout, panel_port, panel_model } =
		CONFIGURATION;
	...
	for (let i = 0; ; i++) {
		...
		const panel_alive = await createPingRequest(panel_ip, panel_port);
		if (panel.socket_destroy_status) {
			console.log(
				"To start the script first terminate all other connections to the panel!",
			);
			break;
		}
		...
		if (!panel_alive) {
			...
			for (let a = 0; ; a++) {
				const panel_revive = await createPingRequest(panel_ip, panel_port);
